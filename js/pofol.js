let data = [
    {
        ...
        desc : `<p>학원 강의 수강 중, 조장으로 선발되어 조원들을 이끌고 진행했던 첫 스프링 팀프로젝트입니다. 전직 디자이너로써 웹 어플리케이션 프로젝트의 진행 방식을 잘 알고 있었기에 프로젝트를 지휘하는 데는 큰 무리가 없었으며, 
        	당시 다른 친구들에 비해 비교적 스프링 숙지가 빠른 편이었기 때문에 프로젝트 설계는 물론 조원들의 스프링 교육까지 같이 진행했습니다.</p>
        	...
        	프로젝트를 진행하며 날짜, 기간을 컨트롤하는 부분은 생각보다 까다롭다는 것을 다시 한번 느꼈습니다.</p>
        	<p>조원 구성 당시 걱정이 많은 상태였으나, 다행히도 조원들이 협조적이어서 결국엔 반에서 가장 안정적으로 스프링 팀 프로젝트를 마무리한 조가 되었습니다. 원래는 프로젝트가 끝나고 팀이 해체될 예정이었으나, 조원들의 요청으로 프로젝트 리드를 한 번 더 하게 됐습니다.</p>
        	<p>단순 디자이너였던 저 역시 GitHub를 써본 적이 없었는데 이 프로젝트를 통해 GitHub사용을 시작했습니다. 위 프로젝트는 저의 원본 저장소를 팀원들이 Fork하여 병합하는 방식으로 협업이 진행되었습니다.</p>`,
        link1 : "https://docs.google.com/presentation/d/1c4kkEKnXA9KJMfWsKUqUxaVw-Sv0bi3YQxIX5buXDRQ/edit?usp=sharing",
    ...
    },
    {
        ...
        frontend : "HTML/CSS, JSTL, JavaScript(JQuery)",
        feature : "Calendar를 이용한 일정 예약, 일정 확인, 일정 취소, 정비소 검색, 대여회사 신청 등",
        desc : `<p>위에서 기술한 프로젝트의 관리자 페이지입니다. 당연하게도 보여지는 사용자 사이트보다 더 많은 작업을 필요로 했고, 예약 테이블 같은 경우 테이블은 1개였으나 핵심 테이블인 만큼 이를 사용하게 될 클라이언트의 편의를 위해 리스트 페이지만 다른 형태로 3번 구현했습니다.</p>
        	<p>그밖에 관리자 대쉬보드에 간단한 통계 쿼리를 구현해 봤습니다.</p>`,
        link1 : "https://docs.google.com/presentation/d/1c4kkEKnXA9KJMfWsKUqUxaVw-Sv0bi3YQxIX5buXDRQ/edit?usp=sharing",
        link2 : "https://github.com/Ahnjaelim/CampingcarGitRefine"	
    },
    {
    ...
    },     
    {
        ...
        desc : `<p>앞서 캠핑카 프로젝트에서 기술했듯, 조원들의 요청으로 한 번 더 리드를 하게 된 프로젝트입니다. 캠핑카 프로젝트가 끝나고 부트를 따로 공부하고 있던 저는 새 프로젝트를 부트로 진행할 것을 제안했습니다. 저 역시 부트를 공부한 지 얼마 되지 않아 조원들과 다 같이 스터디처럼 책 한 권을 두고 부트 공부를 시작했는데, 책에 있던 오탈자를 잡아내는 등 이 과정이 아주 재미있던 걸로 기억합니다.</p>
			<p>확실히 따로따로 공부했던 스프링과 달리 책 한 권을 기준으로 두고 진행한 것과 더불어 스프링 부트를 기반으로 한 프로젝트이다 보니 환경설정면에선 이전 프로젝트에 비해 아주 수월했습니다. 아예 부트에 대한 베이스가 없었기 때문에 학습과 병행한 프로젝트라 이전 팀 프로젝트에 비해 규모는 작은 편입니다.</p>`,
        link1 : "https://docs.google.com/presentation/d/1PKRFed3uvHDPpGevWte1RRwyzgpMdUlgzUz5olcov5s/edit?usp=sharing",
    ...
    }, 
    {
        ...
        desc : `<p>스프링을 막 배우기 시작했을 때 작업한 저의 첫 스프링 프로젝트입니다. 달력 및 일정 기능은 어플리케이션에 있어 필수 기능이라 생각했던 저는, 일정 관리 기능 구현을 목표로 작업을 시작했습니다.</p>
        	...
        	이때 Java의 Calendar 기능을 익히기 시작했고 어느 정도 숙달이 된 후 다음 프로젝트에서 좀 더 디테일하게 날짜를 컨트롤할 수 있게 됐습니다.</p>`,
        link1 : "",
